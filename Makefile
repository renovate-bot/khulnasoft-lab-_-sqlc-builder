...
bootstrap-tools: $(TEMPDIR)
	...
	curl -sSfL https://raw.githubusercontent.com/sulaiman-coder/gobouncer/master/bouncer.sh | sh -s -- -b $(TEMPDIR)/ v0.4.0
	# we purposefully use the latest version of sqlc-builder released
	curl -sSfL https://raw.githubusercontent.com/khulnasoft-labs/sqlc-builder/main/install.sh | sh -s -- -b $(TEMPDIR)/ v0.6.0
...
unit: $(RESULTSDIR) fixtures ## Run unit tests (with coverage)
	...
	go test  -coverprofile $(COVER_REPORT) $(shell go list ./... | grep -v khulnasoft-labs/sqlc-builder/example)
...
CHANGELOG.md:
	$(TEMPDIR)/sqlc-builder > CHANGELOG.md
...
release: clean-dist CHANGELOG.md ## Build and publish final binaries and packages.
	...
	# TODO: in the future add sqlc-builder to generate changelogs
